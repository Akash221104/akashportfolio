...
interface InteractiveBackgroundProps {
  rgbColor?: string; // e.g., "56, 189, 248" for sky blue, "16, 185, 129" for emerald
}

export default function InteractiveBackground({ rgbColor = '56, 189, 248' }: InteractiveBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const renderLoop = () => {
      ...
      if (mouse.active) {
        ...
        grad.addColorStop(0, `rgba(${rgbColor}, 0.18)`);
        grad.addColorStop(0.35, `rgba(${rgbColor}, 0.07)`);
      ...
      particles.forEach((p) => {
        ...
        const isNearCursor =
          mouse.active && Math.sqrt((p.x - mouse.x) ** 2 + (p.y - mouse.y) ** 2) < maxMouseDistance;
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2);
        ctx.fillStyle = isNearCursor ? `rgba(${rgbColor}, 0.75)` : `rgba(${rgbColor}, 0.35)`;
        ctx.fill();
      });
      ...
      sparks = sparks.filter((s) => {
        ...
        ctx.fillStyle = `rgba(${rgbColor}, ${s.alpha})`;
        ctx.fill();
        return true;
      });
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        if (mouse.active) {
          ...
          if (dist < maxMouseDistance) {
            ...
            ctx.beginPath();
            ctx.moveTo(p1.x, p1.y);
            ctx.lineTo(p1.x, p1.y); // subtle node glow anchor
            ctx.strokeStyle = `rgba(${rgbColor}, ${alpha})`;
            ctx.lineWidth = 0.85;
            ctx.stroke();
          }
