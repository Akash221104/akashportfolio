...
export default function AskAkashAI() {
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end gap-2.5 sm:gap-3 max-w-[calc(100vw-32px)]">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-[calc(100vw-32px)] sm:w-[400px] h-[75vh] max-h-[560px] min-h-[360px] rounded-2xl border border-white/10 bg-zinc-950/95 backdrop-blur-2xl shadow-[0_0_60px_rgba(59,130,246,0.15)] flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="p-3.5 sm:p-4 border-b border-white/8 flex items-center justify-between bg-gradient-to-r from-blue-950/60 to-purple-950/60">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center shadow-lg shadow-blue-500/20 shrink-0">
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3.5 sm:space-y-4">
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex gap-2.5 sm:gap-3 max-w-[90%] sm:max-w-[88%] ${msg.sender === 'user' ? 'ml-auto flex-row-reverse' : ''}`}
                >
                  ...
                  <div className={`rounded-2xl px-3.5 sm:px-4 py-2 sm:py-2.5 ${
                    msg.sender === 'user'
              ...
              {isTyping && (
                <div className="flex gap-2.5 sm:gap-3 max-w-[88%]">
                  <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-sky-500/20 text-sky-400 border border-sky-500/20 flex items-center justify-center shrink-0">
                    <Bot className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  </div>
                  <div className="bg-white/5 border border-white/5 rounded-2xl rounded-tl-none px-3.5 sm:px-4 py-2.5 sm:py-3 flex items-center gap-1.5">
            ...
            {suggestions.length > 0 && (
              <div className="px-3 sm:px-4 py-2 border-t border-white/5 overflow-x-auto flex gap-1.5 sm:gap-2 scrollbar-none">
                {suggestions.map((sug, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleSendMessage(sug)}
                    className="shrink-0 px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs bg-white/5 hover:bg-sky-500/10 border border-white/5 hover:border-sky-500/20 text-white/50 hover:text-white transition-all cursor-pointer"
                  >
                    {sug}
                  </button>
                ))}
            ...
            <form
              ...
              className="p-2.5 sm:p-3 border-t border-white/8 bg-black/30 flex items-center gap-2"
            >
              <input
                type="text"
                value={inputValue}
                ...
                className="flex-1 bg-white/5 border border-white/8 rounded-xl px-3.5 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-sky-500/50 transition-colors"
                disabled={isTyping}
              />
              <button
                type="submit"
                disabled={!inputValue.trim() || isTyping}
                className="p-2 sm:p-2.5 rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white disabled:opacity-35 disabled:pointer-events-none transition-all cursor-pointer shadow-lg shadow-blue-500/20"
      ...
      <AnimatePresence>
        {showBubble && !isOpen && (
          <motion.div
            ...
            className="relative flex items-center gap-2 px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-2xl rounded-br-sm bg-gradient-to-r from-sky-500 to-blue-600 text-white text-xs sm:text-sm font-semibold shadow-xl shadow-blue-500/25 cursor-pointer select-none max-w-[260px] sm:max-w-none text-right"
            onClick={handleOpen}
          >
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.93 }}
        className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white shadow-2xl shadow-blue-500/30 flex items-center justify-center cursor-pointer border border-white/10 transition-all duration-300"
        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            >
              ...
              <span className="absolute -top-1 -right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-emerald-400 border-2 border-blue-600 animate-pulse" />
            </motion.div>
          )}
        </AnimatePresence>
      </motion.button>
